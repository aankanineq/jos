...
import { ProfileItem, DEFAULT_GYM_PROFILE } from '@/app/actions/ai-profile-defaults'
...
const STORAGE_KEY = 'jos_ai_profile_gym'
...
export default function GymAISection({ uniqueMonths }: Props) {
  // Profile Items
  ...
  const [isAllMonths, setIsAllMonths] = useState(false) // Default is latest month to avoid bloating
  const [selectedMonths, setSelectedMonths] = useState<string[]>(uniqueMonths.slice(0, 1))
  const [selectedTypes, setSelectedTypes] = useState<string[]>(['Pull', 'Push', 'Leg', 'Full']) // Default Strength Only

  // Tool states
  const [gymMode, setGymMode] = useState<AIMode>('fast')
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      const result = await getAiProfile('gym')
  ...
  const handleSaveProfile = async (nextItems: ProfileItem[]) => {
    ...
    const result = await saveAiProfile('gym', filtered)
  ...
  const handleAddItem = () => {
    const nextId = `gym_custom_${Date.now()}`
  ...
  const handleToggleAllTypes = () => {
    if (selectedTypes.length === ALL_WORKOUT_TYPES.length) {
      setSelectedTypes(['Pull', 'Push', 'Leg', 'Full']) // Reset to default strength
    } else {
    ...
    setChatMessages([
      {
        id: 'welcome',
        role: 'assistant',
        content: '안녕하세요! 당신의 **JOS AI 헬스 코치**입니다. 🏋️\n\n현재 세팅된 내 헬스 프로필과 중량/맨몸 운동 기록을 바탕으로 분석을 지시하거나 질문을 남겨보세요!\n- 아래의 **[근력 운동 심층 분석 실행]** 단축 지시어를 누르면 최근 운동 볼륨과 점진적 과부하 달성도를 정밀 RAG 피드백을 즉시 받아보실 수도 있습니다.',
        timestamp: new Date(),
      }
    ])
  }, [])
  ...
  const handleSendMessage = async (text: string, action?: 'gym_analysis') => {
      ...
      <div className="retro-card overflow-hidden bg-slate-50/20 border border-slate-150 shadow-sm">
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-sm font-extrabold text-slate-800 tracking-tight block leading-tight">내 헬스 프로필 세팅</span>
              <span className="text-[10px] text-slate-400 font-bold">내 운동 스타일, 보충제 정보, 주의점을 AI RAG에 제공</span>
            </div>
          </div>
          {profileOpen ? (
            ...
            {storageMode === 'loading' ? (
              ...
              <>
                <p className="text-[10px] font-bold text-slate-400 leading-relaxed flex items-start gap-1.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <span>
                    나만의 헬스 디테일을 입력해 보세요. AI가 내 목표 부위, 강도 기준, 관절 특이사항을 숙지하여 정밀 처방합니다. 
                  ...
                  {items.map((item, idx) => (
                    <div key={item.item_id} className="flex gap-2 items-center bg-slate-50/50 p-2 rounded-xl border border-slate-100 group animate-in slide-in-from-left-2 duration-150">
                      ...
                      <input
                        ...
                        placeholder="세부 내용 (예: 근비대와 체지방 감량 병행)"
      ...
      <div className="retro-card p-4 sm:p-5 bg-gradient-to-tr from-slate-50/20 to-white border border-slate-150 shadow-sm space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 flex-wrap gap-3">
          <label className="flex items-center gap-2.5 cursor-pointer group select-none">
            ...
            <div>
              ...
              <span className="text-[10px] text-slate-400 font-bold block">분석 시 이전 근력 세션 데이터를 RAG 컨텍스트에 결합시킵니다</span>
            </div>
          </label>
          ...
          <div className="space-y-4 pt-1 animate-in fade-in duration-200">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                >
                  {selectedTypes.length === ALL_WORKOUT_TYPES.length ? '근력운동만 선택' : '전체 선택'}
                </button>
              </div>
              <div className="flex flex-wrap gap-1.5">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between pt-2">
          <div className="space-y-0.5">
            <h3 className="text-sm font-extrabold text-slate-900">AI 점진적 과부하 분석 처방</h3>
            <p className="text-[11px] text-slate-500 font-semibold leading-relaxed">
              나의 과거 운동 세트 및 반복수를 분석하여, 정체기를 극복하고 무게를 점진적으로 늘릴 수 있는 목표 가이드를 수립합니다.
            </p>
          </div>

          <div className="flex bg-slate-100 p-0.5 rounded-xl border border-slate-200 gap-1 shrink-0 sm:w-64">
            <button
              type="button"
              onClick={() => setGymMode('fast')}
              className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-black transition-all cursor-pointer ${
                gymMode === 'fast' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              <Zap className={`w-3 h-3 ${gymMode === 'fast' ? 'text-orange-400 fill-orange-400' : 'text-slate-400'}`} />
              Fast ⚡
            </button>
            <button
              type="button"
              onClick={() => setGymMode('pro')}
              className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-black transition-all cursor-pointer ${
                gymMode === 'pro' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              <Sparkles className={`w-3 h-3 ${gymMode === 'pro' ? 'text-violet-400 fill-violet-400' : 'text-slate-400'}`} />
              Pro ✦
            </button>
            <button
              type="button"
              onClick={() => handleSendMessage('🏋️ [근력 운동 심층 분석 실행] 최근 내 운동 기록과 헬스 프로필을 바탕으로 점진적 과부하 달성도 및 정체기 타파 피드백을 제공해줘.', 'gym_analysis')}
              disabled={chatLoading}
              className="px-4 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-black transition-colors shadow-sm active:scale-97 cursor-pointer shrink-0 disabled:opacity-40"
            >
      ...
      <div className="retro-card overflow-hidden bg-white border border-slate-200 shadow-sm flex flex-col">
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-sm font-black text-slate-800 tracking-tight block">AI 헬스 코치와 대화</span>
              ...
              <div key={msg.id} className="space-y-2">
                ...
                <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-in fade-in duration-200`}>
                  <div className={`rounded-2xl p-4 text-xs font-semibold leading-relaxed shadow-sm max-w-[85%] ${
                    ...
                    {!isUser && (
                      <div className="flex items-center gap-1.5 text-amber-600 font-extrabold tracking-wide uppercase text-[9px] mb-2 border-b border-amber-50 pb-1.5">
            ...
            <div className="flex justify-start animate-pulse">
              <div className="rounded-2xl p-4 bg-white border border-slate-150 shadow-sm rounded-tl-none max-w-[85%]">
                ...
                <div className="flex gap-1 mt-2.5 pl-5">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-bounce duration-300 delay-75" />
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-bounce duration-300 delay-150" />
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-bounce duration-300 delay-225" />
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t border-slate-150/60 bg-slate-50/30 flex flex-col gap-2">
          ...
          >
            ...
            <button
              ...
              className="absolute right-2 top-2 p-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-white shadow transition-all active:scale-95 disabled:opacity-30 cursor-pointer"
            >
